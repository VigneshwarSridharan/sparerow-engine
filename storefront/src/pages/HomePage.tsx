...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <SmartPartFinder onQuickView={onQuickView} />
      ...
      <section className="py-12 bg-muted/30">
        <div className="container">
          ...
          <div className="flex gap-2 flex-wrap">
            {partTypes.slice(0, 12).map((partType) => (
              <Button key={partType} variant="outline" size="sm" className="rounded-full" onClick={() => navigate(`/products?category=${encodeURIComponent(partType)}`)}>
                {partType}
              </Button>
            ))}
          </div>
