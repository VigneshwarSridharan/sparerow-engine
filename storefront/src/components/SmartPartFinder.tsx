...
import xiaomiLogo from '@/assets/brands/xiaomi-logo.png';
...
export function SmartPartFinder({ onQuickView, brands, categories }: SmartPartFinderProps) {
  ...
  const currentStep = selectedPartType ? 3 : selectedModelId ? 2 : selectedBrandId ? 1 : 0;
  ...
  const resetFrom = (step: number) => {
    if (step <= 0) { setSelectedBrandId(null); setSelectedModelId(null); setSelectedPartType(null); }
    else if (step <= 1) { setSelectedModelId(null); setSelectedPartType(null); }
    else if (step <= 2) { setSelectedPartType(null); }
  };
      ...
      <div className="container">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground mt-2 max-w-md mx-auto">
            Select your phone brand, then model, then the part you need — we'll show you everything available.
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-0 mb-10">
          {steps.map((label, i) => (
            <div key={label} className="flex items-center">
              ...
              {i < steps.length - 1 && (
                <div className={cn('w-8 md:w-14 h-0.5 mx-2', i < currentStep ? 'bg-primary' : 'bg-border')} />
              )}
            </div>
          ))}
        </div>
        ...
        <div ref={step1Ref} className="scroll-mt-16 bg-card rounded-2xl border shadow-sm p-6 mb-4">
        ...
        {selectedBrandId && (
          <div ref={step2Ref} className="scroll-mt-16 bg-card rounded-2xl border shadow-sm p-6 mb-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-primary uppercase tracking-wider">Step 2</span>
                <span className="font-semibold text-sm">Select {selectedBrand?.name} Model</span>
              </div>
              <button onClick={() => resetFrom(0)} className="text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            </div>
        ...
        {selectedModelId && (
          <div ref={step3Ref} className="scroll-mt-16 bg-card rounded-2xl border shadow-sm p-6 mb-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-primary uppercase tracking-wider">Step 3</span>
                <span className="font-semibold text-sm">Select Part Type for {selectedModel?.name}</span>
              </div>
              <button onClick={() => resetFrom(1)} className="text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {availablePartTypes.map(pt => (
                ...
                >
                  {getPartImage(pt) ? (
                    <img src={getPartImage(pt)} alt={pt} className="h-8 w-8 object-contain shrink-0 rounded" loading="lazy" />
                  ) : (
        ...
        {/* Step 4: Results */}
        {selectedPartType && (
          <div ref={step4Ref} className="scroll-mt-16 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="bg-gradient-to-r from-primary/90 to-primary rounded-2xl text-primary-foreground text-center p-8 mb-6">
              ...
              <p className="text-primary-foreground/70 text-sm mt-1">
                {resultProducts.length} option{resultProducts.length !== 1 ? 's' : ''} available — browse genuine and aftermarket parts with prices
              </p>
            </div>
            {resultProducts.length > 0 ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <Button variant="outline" className="mt-3" onClick={() => resetFrom(2)}>
